 ...
 *    GET  /tutors?classId=&subjectId=&chapterId=   Tutors for a class+subject+chapter
 *    GET  /posts?classId=&subjectId=&chapterId=&tutorId=   Posts (ordered oldest-first)
 *    GET  /posts/recent?classId=         Top 10 recent posts for a class
 *
 *  ENDPOINTS (authenticated – Firebase Bearer token required):
 *    POST   /tutor/post/create           Create a new post
 *    GET    /tutor/posts                 Fetch own posts (for edit/delete UI)
 *
...
/** Applied to write / sensitive endpoints */
...
function requireBodyFields(body, ...names) {
  for (const name of names) {
    if (!body[name]) throw new Error(`BAD_REQUEST: ${name} is required`);
  }
}
...
const QUEUES = {
  ...
  write:       new Queue(20,  500,   30000), // tutor write endpoints
  tutorRead:   new Queue(30,  1000,  15000), // tutor read endpoints
};
...
// ─────────────────────────────────────────────
// Auth middleware  (Firebase Bearer token)
// ─────────────────────────────────────────────
async function requireFirebaseAuth(req, res, next) {
  ...
  try {
    ...
    logger.warn({ err }, 'Invalid or expired Firebase token');
...
// ─────────────────────────────────────────────
// ENDPOINT 3 — GET /tutors?classId=&subjectId=&chapterId=
//   Returns tutors matching classId + subjectId + chapterId from Firestore.
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// ENDPOINT 4 — GET /posts?classId=&subjectId=&chapterId=&tutorId=
...
app.get('/posts', async (req, res) => {
  try {
    const result = await QUEUES.read.add(async () => {
      requireDb();
      requireQueryParams(req.query, 'classId', 'subjectId', 'chapterId', 'tutorId');
...
// ─────────────────────────────────────────────
// ENDPOINT 5 — GET /posts/recent?classId=
...
// ─────────────────────────────────────────────
// ENDPOINT 6 — POST /tutor/post/create  (auth required)
//   Creates a new post.
//   Also upserts the tutor document in the "tutors" collection.
// ─────────────────────────────────────────────
app.post('/tutor/post/create', strictLimiter, requireFirebaseAuth, async (req, res) => {
  try {
    const result = await QUEUES.write.add(async () => {
      ...
      requireBodyFields(req.body, 'classId', 'subjectId', 'chapterId', 'postTitle', 'videoUrl');
      ...
      if (postSubtitle && postSubtitle.length > 1000)
        throw new Error('BAD_REQUEST: postSubtitle too long (max 1000 chars)');

      // ── Upsert tutor document ──
      const tutorQuery = await db.collection('tutors')
      ...
      // Generate post ID first so we can reference it in the tutor doc
      ...
      if (!tutorQuery.empty) {
        // Tutor already exists for this class/subject/chapter → append postId
        await tutorQuery.docs[0].ref.update({
          postIds:   admin.firestore.FieldValue.arrayUnion(postId),
          updatedAt: now,
        });
      } else {
        // Create new tutor document
        await db.collection('tutors').doc().set({
          tutorId:   userEmail,
          tutorName: userName,
          classId,
          subjectId,
      ...
      await postRef.set({
        ...
        postSubtitle: postSubtitle ? postSubtitle.trim() : '',
        tutorId:      userEmail,
        tutorName:    userName,
      ...
      logger.info({ postId, tutorId: userEmail, classId, subjectId, chapterId }, 'Post created');
...
// ─────────────────────────────────────────────
// ENDPOINT 7 — GET /tutor/posts  (auth required)
//   Fetches all posts uploaded by the authenticated tutor
//   (across all classes/subjects/chapters).
// ─────────────────────────────────────────────
app.get('/tutor/posts', requireFirebaseAuth, async (req, res) => {
  try {
    const result = await QUEUES.tutorRead.add(async () => {
      requireDb();

      const userEmail = req.user.email;

      // 1. Find all tutor documents for this email (may span multiple class/subject/chapter combos)
      ...
      // 2. Collect all postIds across all tutor documents
      const postIdSet = new Set();
      tutorSnap.docs.forEach(doc => {
        const ids = doc.data().postIds;
        if (Array.isArray(ids)) ids.forEach(id => postIdSet.add(id));
      });

      if (postIdSet.size === 0) return [];

      // 3. Firestore "in" queries are limited to 30 items per call – batch them
      const postIds   = [...postIdSet];
      const BATCH     = 30;
      const batches   = [];
      for (let i = 0; i < postIds.length; i += BATCH) {
        batches.push(postIds.slice(i, i + BATCH));
      }
      ...
      // Sort newest first for the edit/delete UI
      posts.sort((a, b) => {
      ...
      logger.info({ tutorId: userEmail, count: posts.length }, 'Tutor posts fetched');
      return posts;
    });
    res.json(result);
  } catch (err) {
